...
$redTagged = $conn->query("SELECT COUNT(*) as count FROM assets WHERE status IN ('damaged', 'unserviceable')")->fetch_assoc()['count'];

// Low quantity items (less than 5)
$lowQtyResult = $conn->query("SELECT asset_name, quantity FROM assets WHERE quantity < 5");

// Assets by Status
$statusStats = $conn->query("SELECT status, COUNT(*) as count FROM assets GROUP BY status");
$statusData = [];
...
<div class="container mt-5">
    <h2 class="mb-4">Asset Analytics Overview</h2>

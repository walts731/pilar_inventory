...
$statusQuery = $conn->query("SELECT DISTINCT filter_status FROM archives WHERE filter_status IS NOT NULL");
while ($row = $statusQuery->fetch_assoc()) {
    $statusOptions[] = $row['filter_status'];
}

$categoryQuery = $conn->query("SELECT DISTINCT filter_category FROM archives WHERE filter_category IS NOT NULL");
            ...
            <div class="table-responsive">
                <table id="archiveTable" class="table table-bordered table-striped w-100">
                    ...
                    if ($archivesQuery->num_rows > 0) {
                        ...
                        echo "<tr><td colspan='8' class='text-center'>No archives found.</td></tr>";
                    }
                    ?>
                    </tbody>
                </table>

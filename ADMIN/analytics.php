...
// Assuming the office_id is passed through the session or query string
$office_id = isset($_SESSION['office_id']) ? $_SESSION['office_id'] : (isset($_GET['office_id']) ? $_GET['office_id'] : null);
...
$statusData = [];
$statusQuery = "SELECT status, COUNT(*) AS count FROM assets WHERE office_id = ? GROUP BY status";
...
$lowStockQuery = "SELECT COUNT(*) AS low_stock FROM assets WHERE office_id = ? AND quantity <= 3";
...
$valueQuery = "SELECT SUM(value * quantity) AS total_value FROM assets WHERE office_id = ?";
$stmt = $conn->prepare($valueQuery);
$stmt->bind_param('i', $office_id);
$stmt->execute();
$valueResult = $stmt->get_result();
...
$restockQuery = "SELECT asset_name, quantity, category FROM assets WHERE office_id = ? AND quantity < 5 ORDER BY quantity ASC";
$stmt = $conn->prepare($restockQuery);
$stmt->bind_param('i', $office_id);
$stmt->execute();
$restockResult = $stmt->get_result();
      ...
      <tbody>
        <?php foreach ($restockSuggestions as $item) : ?>
          <tr>
            ...
            <td><?= $item['quantity'] ?></td>
            <td>
              <?php if ($item['category'] === 'Office Supplies'): ?>
                <?= $item['category'] ?> - <?= $item['quantity'] == 0 ? 'Urgent Restock' : 'Consider Restocking' ?>
              <?php else: ?>
                -
              <?php endif; ?>
            </td>
